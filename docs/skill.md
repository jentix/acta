...
Writes `.claude/skills/acta-document/SKILL.md` and adds an
...
project's `.claude/skills/acta-document/` directory.
...
- **Version-locked delivery.** The skill is bundled in `@acta-dev/cli`, so the skill
  a user gets from `acta init --skill` is exactly the one matching their installed
  CLI version.
