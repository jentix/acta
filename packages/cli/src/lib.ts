...
import {
  ...
  buildSearchIndex,
  buildSiteMeta
} from "@adr-book/renderer";

const DEFAULT_CONFIG: AdrBookConfig = {
  title: "ADR Book",
...
export function initWorkspace(cwd: string): string[] {
  ...
  writeIfMissing(
    ...
    createDocumentTemplate("adr")
  );
  writeIfMissing(
    path.join(cwd, DEFAULT_CONFIG.templatesDir, "spec.md"),
    createDocumentTemplate("spec")
  );
  writeIfMissing(
    path.join(cwd, DEFAULT_CONFIG.decisionsDir, "adr-0001-use-markdown-documents-with-yaml-frontmatter.md"),
    `---
id: ADR-0001
kind: adr
...
`
  ...
  writeIfMissing(
    path.join(cwd, DEFAULT_CONFIG.specsDir, "spec-0001-adr-book-document-model.md"),
    `---
id: SPEC-0001
kind: spec
...
export function createNewDocument(
  ...
  const fileDir = path.join(
    cwd,
    kind === "adr" ? config.decisionsDir : config.specsDir
  );
  const document = {
    id: nextId,
    title,
  ...
  writeFileSync(
    filePath,
    fillTemplate(createDocumentTemplate(kind), templateValues),
    "utf8"
  );
  return filePath;
}

export function formatDocumentTable(index: RepositoryIndex): string {
...
export function formatDocumentDetails(index: RepositoryIndex, id: string): string {
  ...
  return [
    ...
    "links:",
    ...Object.entries(document.links).map(
      ([type, values]) => `  ${type}: ${Array.isArray(values) && values.length > 0 ? values.join(", ") : "-"}`
    ),
...
export function formatIssues(index: RepositoryIndex): string {
...
export function graphAsMermaid(index: RepositoryIndex): string {
  ...
  for (const edge of index.graph.edges) {
    lines.push(
      `  ${sanitizeMermaidId(edge.from)} -->|${edge.type}| ${sanitizeMermaidId(edge.to)}`
    );
  }
  return lines.join("\n");
}

export async function buildArtifacts(cwd: string): Promise<string[]> {
  const config = loadConfig(cwd);
  const index = buildRepositoryIndex(cwd, config);
  const builtDocuments = await buildDocumentArtifacts(
    index.documents,
    index.backlinks,
    index.issues
  );
...
export function loadIndex(cwd: string): RepositoryIndex {
  const config = loadConfig(cwd);
...
function createDocumentTemplate(kind: DocumentKind): string {
  if (kind === "adr") {
    return `---
id: {{id}}
kind: adr
...
function fillTemplate(
  template: string,
  values: { id: string; title: string; date: string; owner?: string }
): string {
  return template
