...
import { createFixture, adrContent } from "./fixture.js";
...
describe("acta new adr", () => {
  it("creates ADR-0001 when no ADRs exist", async () => {
    ...
    try {
      ...
      await newCommand.subCommands.adr.run({ args: { title: "My First Decision", config: undefined, id: undefined, status: undefined, tags: undefined } });
      ...
      const content = await readFile(join(fixture.config.resolvedDocs.adrDir, files[0] ?? ""), "utf8");
  ...
  it("allocates ADR-0002 when ADR-0001 exists", async () => {
    ...
    try {
      ...
      const { newCommand } = await import("../commands/new.js");
      // @ts-expect-error citty internal
      await newCommand.subCommands.adr.run({ args: { title: "Second Decision", config: undefined, id: undefined, status: undefined, tags: undefined } });
  ...
  it("refuses to create when file already exists", async () => {
    ...
    try {
      ...
      // Create once
      // @ts-expect-error citty internal
      await newCommand.subCommands.adr.run({ args: { title: "Collision", config: undefined, id: undefined, status: undefined, tags: undefined } });

      // Same id / slug would collide if file exists
      await expect(
        // @ts-expect-error citty internal
        newCommand.subCommands.adr.run({ args: { title: "Collision", config: undefined, id: "ADR-0001", status: undefined, tags: undefined } }),
      ).rejects.toThrow();
  ...
  it("creates SPEC with spec kind", async () => {
    ...
    try {
      ...
      const { newCommand } = await import("../commands/new.js");
      // @ts-expect-error citty internal
      await newCommand.subCommands.spec.run({ args: { title: "Auth System", config: undefined, id: undefined, status: undefined, tags: undefined } });
      ...
      const content = await readFile(join(fixture.config.resolvedDocs.specDir, files[0] ?? ""), "utf8");
