...
function makeRepo(): string {
  ...
  writeFileSync(
    path.join(cwd, "adr-book.config.json"),
    JSON.stringify(
      {
        title: "Test ADR Book",
...
describe("core repository index", () => {
  it("parses valid adr and spec and builds backlinks", () => {
    const cwd = makeRepo();
    writeFileSync(
      path.join(cwd, "docs/decisions/adr-0001-markdown.md"),
      `---
id: ADR-0001
kind: adr
title: Use Markdown
...
`
    ...
    writeFileSync(
      path.join(cwd, "docs/specs/spec-0001-model.md"),
      `---
id: SPEC-0001
kind: spec
...
`
  ...
  it("reports duplicate ids, broken refs and missing sections", () => {
    const cwd = makeRepo();
    writeFileSync(
      path.join(cwd, "docs/decisions/one.md"),
      `---
id: ADR-0001
kind: adr
title: One
...
`
    ...
    writeFileSync(
      path.join(cwd, "docs/specs/two.md"),
      `---
id: ADR-0001
kind: spec
title: Two
...
`
  ...
  it("detects supersedes cycles and implemented spec warning", () => {
    const cwd = makeRepo();
    writeFileSync(
      path.join(cwd, "docs/decisions/adr-1.md"),
      `---
id: ADR-0001
kind: adr
title: One
...
`
    ...
    writeFileSync(
      path.join(cwd, "docs/decisions/adr-2.md"),
      `---
id: ADR-0002
kind: adr
title: Two
...
`
    ...
    writeFileSync(
      path.join(cwd, "docs/specs/spec-1.md"),
      `---
id: SPEC-0001
kind: spec
