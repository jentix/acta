...
- `/search` canonical search page with shareable `?q=` URLs.

### Web implementation principles

- Viewer exposes and consumes the core `search-index.json` artifact shape for browser search.
- Static HTML first.
- Minimal client JS.
- Graph interactivity can use small vanilla SVG behavior unless a dedicated graph library becomes necessary.
- Orama powers browser search from `/search-index.json`.
...
- Static Astro app.
- Browser search backed by `search-index.json`.
- Dashboard.
- Document list.
- Document page.
- Graph page.
